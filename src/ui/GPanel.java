...
public class GPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	/**
	 * Clear the start and end realizations for generating paths
	 */
	 ...
	 */
	private void drawBase(Realization realization, Graphics2D g2d) {
		g2d.setStroke(MyStrokes.dashed);
		g2d.setColor(Color.blue);
		assert (tdModel.isTd());
		Edge base = tdModel.getBaseNonedge();
		assert (base != null);
		drawEdge(realization, base, g2d);
		if (!control.isDrawing()) {
			drawEdgeLength(realization, base, g2d);
		}
	}
	...
	/**
	 * Draws the length of the (non)edge (v,u) upon the edge
	 */
	private void drawEdgeLength(Realization graph, Vertex v, Vertex u,
			Graphics2D g2d) {
			...
			for (Vertex u : graph.getNeighbors(v)) {
				if (control.isShowingCluster()) {
					...
					if (c != null) {
						...
						g2d.setColor(Util.randomColor(c.hashCode()));
					}
				} else {
					// set color according to solution type
					Color c = Color.red;
					if (tdModel.isTd()) {
						RealizationType t = tdModel.getForwardSolutionType(r);
	...
	/**
	 * Draw the part of the linkages constructed by construction steps up to
	 * showStep
	 */
			...
			for (int i = 0; i < vs.size(); ++i) {
				...
				for (int j = 0; j < i; ++j) {
					...
					if (tdModel.isAdjacent(v, u)) {
						drawEdge(tdModel.getCurRealization(), v, u, g2d);
					}
				}
				drawVertex(tdModel.getCurRealization(), v, g2d);
			}
		}
	...
	private void drawTrace(Graphics2D g2d) {
		...
		for (int index = 0; index < traceModel.getTracingVertices().size(); ++index) {
			...
			if (!traceCreated) {
				Trace trace = new Trace(tracingVertex,
						tdModel.getTd().cayleyConfigSpace.getOrientedCCS(t)
								.getContainingInterval(lf), t, tdModel.getTd());
				trace.addPoint(pv, lf);
	...
	public void paint(Graphics g) {
		...
		if (control.isTracingMotion() == 3) {
			...
			Realization g1 = motionModel.getSpinnerModel1().getValue()
					.getValue();
			...
			drawRealization(g11, g2d);

			Realization g2 = motionModel.getSpinnerModel2().getValue()
					.getValue();
			...
			drawRealization(g22, g2d);
		} else if (!isHidingMainRealization)
			// Draw the main realization
			drawRealization(tdModel.getCurRealization(), g2d);
		...
		if (FixAxisListener.getInstance().getO() != null) {
			...
			this.drawVertex(tdModel.getCurRealization(), FixAxisListener.getInstance()
					.getO(), g2d);
		}
	}
	...
	private void drawCompleteCayley(Graphics2D g2d) {
		for (Edge e : tdModel.getTd().completeCayleyVector()) {
			...
			drawEdge(tdModel.getCurRealization(), e, g2d);
		}
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getClickCount() == 3 && ccsModel.isGenerated()) {
			ConfigPanel c = new ConfigPanel(main, tdModel.getCurRealization());
			graphPanel.add(c);
