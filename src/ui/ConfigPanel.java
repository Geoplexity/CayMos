...
/**
 * Panels that stores and shows a thumbnail of a specific realization
 *
 */
public class ConfigPanel extends JPanel implements MouseMotionListener,
	...
	public ConfigPanel(Main m, Realization g, double ratio, boolean isThumb) {
		...
		realization = g.clone(); // ???
		realization.transformVertices(t);
	...
	public void paint(Graphics g) {
		...
		LinkageGraph graph = realization.getGraph();
		...
		for (int i = 0; i < realization.getSize(); ++i) {
			...
			for (Vertex u : graph.getNeighbors(v)) {
				Color c = Color.red;
				if (tdModel.isTd()) {
					RealizationType t = tdModel
							.getForwardSolutionType(realization);
					c = t.getColor();
				}
				g2d.setColor(c);
				Point2D pv = realization.getPoint(v), pu = realization
						.getPoint(u);
		...
		if (tdModel.isTd()) {
			...
			Point2D pv = realization.getPoint(base.v1()), pu = realization
					.getPoint(base.v2());
